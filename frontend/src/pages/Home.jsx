...
import { Search, Zap, Filter, TrendingUp, TrendingDown, Minus, ArrowRight } from 'lucide-react'
...
export default function Home() {
  ...
  return (
    <div className="sm:ml-32 -mt-6 -mx-4">
      {/* ─── HERO ─── */}
      <div className="relative overflow-hidden" style={{ minHeight: 420, background: '#060d1a' }}>
        {/* Globe */}
        <GlobeCanvas />
        ...
        {/* Radial vignette left */}
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 30% 50%, rgba(6,13,26,0) 0%, rgba(6,13,26,0.85) 70%)' }} />
        {/* Bottom fade */}
        <div className="absolute bottom-0 left-0 right-0 h-24 pointer-events-none"
          style={{ background: 'linear-gradient(to bottom, transparent, #060d1a)' }} />

        {/* Content */}
        <div className="relative z-10 px-6 py-14 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold mb-5 border border-brand/30"
            style={{ background: 'rgba(56,189,248,0.08)', color: '#38bdf8' }}>
            <span className="w-1.5 h-1.5 rounded-full bg-brand animate-pulse" />
            ANÁLISIS EN TIEMPO REAL
          </div>

          <h1 className="font-black uppercase leading-none mb-5 tracking-tight"
            style={{ fontSize: 'clamp(2.2rem, 5vw, 3.8rem)', color: '#f1f5f9' }}>
            RECOMENDACIONES<br />
            <span style={{ color: '#38bdf8' }}>BURSÁTILES</span><br />
            DISRUPTIVAS
          </h1>

          <p className="text-slate-400 mb-7 max-w-md text-sm leading-relaxed">
            IA que analiza fundamentos, técnicos y macro en segundos para darte la ventaja que los institucionales tienen.
          </p>
          ...
          <form onSubmit={handleAnalyze} className="flex gap-3 max-w-lg">
            ...
            />
            <button type="submit" disabled={loading || !ticker.trim()} className="btn-primary flex items-center gap-2 whitespace-nowrap px-6">
              {loading
          ...
          {loading && <p className="text-slate-500 text-xs mt-3 animate-pulse">Generando análisis con IA...</p>}
        </div>
      </div>
